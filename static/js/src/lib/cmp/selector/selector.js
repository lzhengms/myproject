    ...
    var Selector = Widget.extend({
        ...
        bindEvents: function() {
            ...
            optionContainer.on(this.get('triggerType'), normalizedClassName, function(e) {
                self.select($(e.target), e.ctrlKey || e.metaKey);
            });
        },
        select: function(target, isMultiSelect) {
            // selectable
            var number = this.get('number');
            var options = this.get('options');
            ...
            if (indexOf(this.get('unselectable'), option) !== -1) {
                return this.trigger('unselectable', target, option, selected, isMultiSelect);
            }
            ...
            this.trigger('select', target, option, selected, isMultiSelect);
            return selected;
        },
