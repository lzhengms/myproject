...
define(function(require, exports, module) {
    ...
    var Upload = Base.extend({
        ...
        initialize: function() {
            ...
            /*settings.use_multipart = this.get('use_multipart');
            settings.use_chunk = this.get('chunk');
            settings.chunk_size = this.get('bytesPerChunk');
            settings.use_socket = this.get('use_socket');*/
            ...
            settings.file_queue_error_handler = function(file, code, message) {
                if(code == SWFUpload.QUEUE_ERROR.QUEUE_LIMIT_EXCEEDED) {
                    return that.trigger('overCountLimit', settings.file_queue_limit);
                } else if(code == SWFUpload.QUEUE_ERROR.FILE_EXCEEDS_SIZE_LIMIT) {
                    return that.trigger('overSizeLimit', settings.file_size_limit, file);
                } else if(code == SWFUpload.QUEUE_ERROR.ZERO_BYTE_FILE) {
                    return that.trigger('zeroSize', file);
                } else if(code == SWFUpload.QUEUE_ERROR.INVALID_FILETYPE) {
                    return that.trigger('notAllowType', file);
                }
        ...
        _onChangeData:function(val){
            this.swfupload.setPostParams(val);
        }
    });
